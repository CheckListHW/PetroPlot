...
class Model():
    def __init__(self, las_path, Biot=0.85):
        ...
        self.Geomech_Model = pd.DataFrame({ 'Sv':self.las['SV'],'SHmax':self.las['SH_MAX_V'],
                                 ...
                                 'Poisson_ratio':self.las['POISON'], 'TENSILE_STRENGTH':self.las['TENSILE_STRENGTH'],
                                 'UCS':self.las['CO_BEFORE_CALIBRATION'],'TVD':self.las['TVD'],'BIOT':self.las['BIOT'],
                                 'mi':self.las['MI'], 'E':self.las['E'],'SHmax_azimuth':self.las['SH_MAX_AZIMUTH'],
                                 'BS':self.las['BS'],'CALIPER':self.las['CALIPER'],'MUD_DENS':self.las['MUD_DENS'] }, index = self.las['DEPT'])
    ...
    def Mud_loss_classify(self, Breakout_classification):
        index=self.Geomech_Model.index
        Mud_loss_classification=pd.DataFrame({'Mud_loss_classification':Breakout_classification.Breakout_classification},
                                         index=index)
    ...
    def Transform_Stress(self, Sv, SHmax, Shmin, SHmax_azimuth, Well_azimuth_input, Well_deviation_input, Degrees=True):
        ...
        if not Degrees:
            ...
            Azimuth_from_SHmax=np.where(Well_azimuth_input>SHmax_azimuth,
                                    2*np.pi-(Well_azimuth_input-SHmax_azimuth), SHmax_azimuth-Well_azimuth_input)
    ...
    def Kirsch_Wall(self, Sxo, Syo, Szo, txyo, tyzo, tzxo, Ppore, Pw, Poisson_ratio):
        ...
        b=Szo-Poisson_ratio*(2*(Sxo-Syo)*np.cos(2*np.radians(i))+4*txyo*np.sin(2*np.radians(i)))-Ppore
        ...
        for i in range(1,180):
            St_x=Sxo+Syo-2*(Sxo-Syo)*np.cos(2*np.radians(i))-4*txyo*np.sin(2*np.radians(i))-Pw-Ppore
            Sz_x=Szo-Poisson_ratio*(2*(Sxo-Syo)*np.cos(2*np.radians(i))+4*txyo*np.sin(2*np.radians(i)))-Ppore
    ...
    def Coulumb_breakout(self, S_1, S_3, UCS, mi, Smax_x_i, Ppore, TVD):
        ...
        Breakout_grad=((Smax_x_i-UCS.loc[:,:90])/(k_factor.loc[:,:90])+Ppore.loc[:,:90])/(TVD.loc[:,:90]*9.81)*1000
...
#            print(self.progress_iterator)
    ...
    def define_success(self, ratio, Geomech_Model, Breakout_classification, 
        ...
        df['Success_'+str(ratio)]=np.where((df['Breakout_classification']==3), np.nan, df['Success_'+str(ratio)])
    ...
    def Define_Strains(self, Breakout_classification, Mud_loss_classification, 
        ...
        for ratio in ratio_list:
            ...
            df['Shmin_ratio_'+str(ratio)] = (df['v']*df['Sv']/(1-df['v']) - df['v']*Biot*df['Pp']/(1-df['v']) + Biot*df['Pp'] +        
                                        df['E']*Strain_min/(1-df['v']**2) + df['v']*df['E']*Strain_max/(1-df['v']**2))
            df['SHmax_ratio_'+str(ratio)] = (df['v']*df['Sv']/(1-df['v']) - df['v']*Biot*df['Pp']/(1-df['v']) + Biot*df['Pp'] +
    ...
    def UCS_calibrate(self, Co, mi, Caliper, BS, Breakout_classification, Mud_dens, 
        ...
        df['Co_calibrated']=np.where((df['Mud_dens']<df['Breakout_grad_0']) & 
                                     (df['Breakout_classification']==0),df['Co']-df['dC_0']*1.1, df['Co_calibrated'])
        df['Co_calibrated']=np.where((df['Mud_dens']>df['Breakout_grad_0']) & 
                                     (df['Breakout_classification']==2),df['Co']-df['dC_90']*1.1, df['Co_calibrated'])
        df['Co_calibrated']=np.where((df['Mud_dens']<df['Breakout_grad_45']) & 
                                     (df['Breakout_classification']==0),df['Co']-df['dC_0']*1.1, df['Co_calibrated'])
        df['Co_calibrated']=np.where((df['Mud_dens']<df['Breakout_grad_0']) & (df['Mud_dens']>df['Breakout_grad_45']) &
    ...
    def Calibrate(self, MD=3388, Pc=60.37):
        ...
        UCS_calibrated=self.UCS_calibrate(Co=self.Geomech_Model.UCS, mi=self.Geomech_Model.mi,
                                ...
                                Breakout_classification=Breakout_classification,
                                Mud_dens=self.Geomech_Model.MUD_DENS, Breakout_grad=Breakout_grad_strain_calibrated,
        ...
        Success=self.define_success(ratio=best_ratio, Geomech_Model=self.Geomech_Model, Breakout_classification=Breakout_classification, 
                       Mud_loss_classification=Mud_loss_classification, Breakout_grad=Breakout_grad_calibrated, 
...
# Group1 Frame ----------------------------------------------------
group1 = LabelFrame(window, text="Предварительный просмотр las файлов")
group1.grid(row=1, column=0,  padx=(5), pady=0, columnspan=2, sticky=E+S+N+W)
